...
const Blog = () => {
    return (
        ...
        <p className="card-text " style={{fontSize:'19px', textAlign:'justify'}}>Cross-Origin Resource Sharing (CORS) is an HTTP-header based mechanism that allows a server to indicate any origins (domain, scheme, or port) other than its own from which a browser should permit loading resources. CORS also relies on a mechanism by which browsers make a "preflight" request to the server hosting the cross-origin resource, in order to check that the server will permit the actual request. In that prefligh.</p>
...
<div className="card mt-5  " style={{ backgroundColor:'#E8F6F3 '}} >
      ...
      <div className="card-body">
        ...
        <p className="card-text" style={{fontSize:'19px',textAlign:'justify'}}> The private route component is similar to the public route, the only change is that redirect URL and authenticate condition.If the user is not authenticated he will be redirected to the login page and the user can only access the authenticated routes If he is authenticated (Logged in).</p>
